...
export class H264ToTsEncoder {
    ...
    public encodeFrame(h264Data: Buffer): Buffer[] {
        ...
        while (offset < pesPacket.length) {
            ...
            const tsPacket = this.createTsPacket(pesPacket, offset, isFirst);
    ...
    private createPesPacket(h264Data: Buffer): Buffer {
        ...
        const naluWithStartCode = Buffer.concat([Buffer.from([0x00, 0x00, 0x00, 0x01]), h264Data]);
    ...
    private writePtsDts(buffer: Buffer, offset: number, pts: number, flags: number): void {
        const byte0 = (flags << 4) | (((pts >> 30) & 0x07) << 1) | 1;
        buffer.writeUInt8(byte0, offset);
        offset++;

        const byte1 = (((pts >> 15) & 0xff) << 1) | 1;
        buffer.writeUInt16BE((byte1 << 8) | 1, offset);
        offset += 2;

        const byte2 = ((pts & 0x7fff) << 1) | 1;
     ...
     * 创建单个 TS 包
     */
    private createTsPacket(data: Buffer, dataOffset: number, isFirst: boolean): Buffer {
